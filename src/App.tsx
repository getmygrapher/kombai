...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      ...
      if (hasDraft && !path.startsWith('/onboarding') && !isPublicPath) {
        // Redirect unauthenticated users with draft to welcome, but don't override public routes
        navigate('/welcome');
      }
    }
  }, [isAuthenticated, loadDraft, navigate, location.pathname]);
